...
function freeText(id: string, x: number, y: number, lineCount = 3): FreeTextInteraction {
  const band = 0.0225;
  return {
    id,
    kind: 'free-text',
...
describe('exercise answer persistence', () => {
  ...
  it('persists and restores free-text answers keyed by interaction id', () => {
    const freeTexts = [
      freeText('free-text-1', 0.2, 0.3, 3),
      freeText('free-text-2', 0.2, 0.36, 2),
    ];
    writeAnswersForPage('book-a', 28, {
      'free-text-1': 'Synthetic answer one',
      'free-text-2': 'Synthetic answer two',
    }, [], [], [], [], [], freeTexts, SCHEMA, storage);

    const restored = readAnswersForPage('book-a', 28, [], [], [], [], [], freeTexts, SCHEMA, storage);
  ...
  it('keeps free-text answers isolated by book and page', () => {
    const freeTexts = [freeText('free-text-1', 0.2, 0.3, 3)];
    writeAnswersForPage('book-a', 28, { 'free-text-1': 'Synthetic page answer one' }, [], [], [], [], [], freeTexts, SCHEMA, storage);
    writeAnswersForPage('book-a', 29, { 'free-text-1': 'Synthetic page answer two' }, [], [], [], [], [], freeTexts, SCHEMA, storage);

    expect(readAnswersForPage('book-b', 28, [], [], [], [], [], freeTexts, SCHEMA, storage)).toEqual({});
    expect(readAnswersForPage('book-a', 28, [], [], [], [], [], freeTexts, SCHEMA, storage))
      .toEqual({ 'free-text-1': 'Synthetic page answer one' });
    expect(readAnswersForPage('book-a', 29, [], [], [], [], [], freeTexts, SCHEMA, storage))
      .toEqual({ 'free-text-1': 'Synthetic page answer two' });
  });

  it('ignores stale free-text answers when the interaction geometry changes', () => {
    const before = [freeText('free-text-1', 0.2, 0.3, 3)];
    writeAnswersForPage('book-a', 28, { 'free-text-1': 'Synthetic stale answer' }, [], [], [], [], [], before, SCHEMA, storage);

    const after = [freeText('free-text-1', 0.2, 0.42, 3)];
    expect(readAnswersForPage('book-a', 28, [], [], [], [], [], after, SCHEMA, storage)).toEqual({});
  });

  it('ignores stale free-text answers when the response line count changes', () => {
    const before = [freeText('free-text-1', 0.2, 0.3, 3)];
    writeAnswersForPage('book-a', 28, { 'free-text-1': 'Synthetic stale answer' }, [], [], [], [], [], before, SCHEMA, storage);

    const after = [freeText('free-text-1', 0.2, 0.3, 4)];
    expect(readAnswersForPage('book-a', 28, [], [], [], [], [], after, SCHEMA, storage)).toEqual({});
  });

  it('drops cleared free-text answers instead of restoring them', () => {
    const freeTexts = [freeText('free-text-1', 0.2, 0.3, 3)];
    writeAnswersForPage('book-a', 28, { 'free-text-1': 'Synthetic answer' }, [], [], [], [], [], freeTexts, SCHEMA, storage);
    writeAnswersForPage('book-a', 28, { 'free-text-1': '' }, [], [], [], [], [], freeTexts, SCHEMA, storage);

    expect(readAnswersForPage('book-a', 28, [], [], [], [], [], freeTexts, SCHEMA, storage)).toEqual({});
  });

  it('keeps all six interaction kinds on the same page independently', () => {
    const blanks = [blank('blank-1', 0.2, 0.3, 0.1)];
    const choices = [choice('choice-1', 0.5, 0.6)];
    const grids = [grid('grid-1', [gridRow('grid-1-row-1', 0.4)])];
    const orderings = [ordering('ordering-1', 0.2, 0.75)];
    const matchings = [matching('matching-1', 0.2, 0.8)];
    const freeTexts = [freeText('free-text-1', 0.2, 0.9, 3)];
    writeAnswersForPage(
      'book-a',
      49,
      {
        ...
        'ordering-1': 'ordering-1-item-4,ordering-1-item-1',
        'matching-1': '{"matching-1-left-2":"matching-1-right-1"}',
        'free-text-1': 'Synthetic free-text answer',
      },
      blanks,
      choices,
      grids,
    ...
    const restored = readAnswersForPage('book-a', 49, blanks, choices, grids, orderings, matchings, freeTexts, SCHEMA, storage);
    expect(restored).toEqual({
      'blank-1': 'ist',
      'choice-1': 'group-1-3',
      'grid-1-row-1': 'grid-group-1-nein',
  ...
  it('produces a stable fingerprint from free-text geometry, line count, and schema', () => {
    const a = freeText('free-text-1', 0.200004, 0.300005, 4);
    const b = freeText('free-text-1', 0.2, 0.3, 4);
    expect(freeTextFingerprint(a, SCHEMA)).toBe(freeTextFingerprint(b, SCHEMA));
    expect(freeTextFingerprint(freeText('free-text-1', 0.2, 0.3, 5), SCHEMA))
      .not.toBe(freeTextFingerprint(b, SCHEMA));
    expect(freeTextFingerprint(a, '0.3.0')).not.toBe(freeTextFingerprint(b, SCHEMA));
  });
});
