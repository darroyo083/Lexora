...
function Status({ correct, neutral = false }: { correct: boolean | null; neutral?: boolean }) {
  ...
  return <p className="showcase-status" role="status" data-state={neutral ? 'neutral' : correct ? 'correct' : 'retry'}>{neutral ? 'Saved locally. Open responses use AI-assisted feedback, not an automatic grade.' : correct ? 'Correct. The canonical answer matches.' : 'Try again. The canonical answer is different.'}</p>;
}
...
export default function InteractionShowcase() {
  ...
  return <section className="interaction-showcase" aria-labelledby="showcase-title"><div className="section-copy"><h2 id="showcase-title">Try the interaction language.</h2><p>Canonical answers use deterministic checks. True open responses can receive bounded AI-assisted feedback, clearly separate from automatic grading.</p></div><div className="showcase-layout"><div className="showcase-tabs" role="tablist" aria-label="Exercise families">{families.map((item) => <button type="button" key={item} role="tab" aria-selected={family === item} onClick={() => selectFamily(item)}>{item}</button>)}</div><div className="showcase-stage" role="tabpanel" aria-label={`${family} example`}><div className="showcase-stage-body" key={family}><div className="showcase-stage-heading"><span>Example exercise</span><h3>{family}</h3></div>{family === 'Fill blank' && <FillBlankExample />}{family === 'Choice' && <ChoiceExample />}{family === 'Choice grid' && <GridExample />}{family === 'Sentence ordering' && <OrderingExample />}{family === 'Matching' && <MatchingExample />}{family === 'Free text' && <FreeTextExample />}</div></div></div></section>;
}
