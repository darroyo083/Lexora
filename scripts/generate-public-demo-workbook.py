...
def header(draw: ImageDraw.ImageDraw, unit: str, title: str, subtitle: str, page: int) -> int:
    draw.text((MARGIN, 64), "LEXORA  /  DEUTSCH A1-A2", font=F["eyebrow"], fill=GREEN)
...
def footer(draw: ImageDraw.ImageDraw) -> None:
    draw.text((MARGIN, HEIGHT - 62), "Synthetic demo material created for Lexora", font=F["small"], fill=MUTED)
    draw.text((WIDTH - MARGIN, HEIGHT - 62), "lexora.dev", font=F["small"], fill=GREEN, anchor="ra")


def page_one() -> Image.Image:
    image = Image.new("RGB", (WIDTH, HEIGHT), PAPER)
    d = ImageDraw.Draw(image)
    y = header(d, "Unit 1", "Mein Morgen", "Build a small morning routine with familiar verbs and time phrases.", 1)

    body = card(d, y, 390, "1", "Lücken ergänzen", "Complete each sentence with the verb in brackets.")
    prompts = [
    ...
    y += 422
    body = card(d, y, 390, "2", "Die passende Antwort", "Choose one answer for each question.")
    questions = [
    ...
    y += 422
    body = card(d, y, 250, "3", "Kurz schreiben", "Write two sentences about your own morning.")
...
def page_two() -> Image.Image:
    image = Image.new("RGB", (WIDTH, HEIGHT), PAPER)
    d = ImageDraw.Draw(image)
    y = header(d, "Unit 2", "In der Stadt", "Use articles and everyday places to describe a short trip through town.", 2)

    body = card(d, y, 430, "4", "Artikel waehlen", "Mark the correct article for each noun.")
    ...
    d.text((MARGIN + 50, body + 350), "Example: der Park", font=F["small"], fill=MUTED)

    y += 462
    body = card(d, y, 510, "5", "Wo findet man das?", "Connect each place with the thing you can find there.")
    ...
    d.text((MARGIN + 42, body + 342), "Draw four lines. Every place has one partner.", font=F["small"], fill=MUTED)

    y += 542
    body = card(d, y, 200, "6", "Ein Satz", "Complete the route with one suitable place.")
...
def page_three() -> Image.Image:
    image = Image.new("RGB", (WIDTH, HEIGHT), PAPER)
    d = ImageDraw.Draw(image)
    y = header(d, "Unit 3", "Ein Treffen planen", "Put messages in order and choose useful phrases for making plans.", 3)

    body = card(d, y, 430, "7", "Satzpuzzle", "Put the word cards in the correct order.")
    token_rows = [
    ...
    y += 462
    body = card(d, y, 430, "8", "Welche Nachricht passt?", "Choose the best reply.")
    prompts = [
    ...
    y += 462
    body = card(d, y, 300, "9", "Deine Nachricht", "Write a short invitation with a day, time, and place.")
...
def page_four() -> Image.Image:
    image = Image.new("RGB", (WIDTH, HEIGHT), PAPER)
    d = ImageDraw.Draw(image)
    y = header(d, "Unit 4", "Kleine Wiederholung", "Review the language from the workbook in a final practical page.", 4)

    body = card(d, y, 420, "10", "Verbformen", "Complete the mini-dialogue with the correct verb forms.")
    lines = [
    ...
    y += 452
    body = card(d, y, 390, "11", "Richtig oder falsch?", "Read the note and mark the correct answer.")
    ...
    y += 422
    body = card(d, y, 320, "12", "Mini-Ziel", "Write one concrete sentence about your next German practice.")
